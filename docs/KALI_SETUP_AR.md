...
| Python | **3.10 – 3.13** | ✅ كالي 2025.2 → 2026.1 يأتي افتراضيًا بـ **Python 3.13** (مدعومة). ⚠️ لو ظهرت **3.14** (تحديثات rolling الأحدث) استخدم miniforge بـ 3.12 (انظر «حل المشاكل» فقرة 3) |
...
**السبب:** TensorFlow يدعم رسميًا Python **3.10 – 3.13** فقط. معظم إصدارات كالي
(2025.2 حتى 2026.1) تأتي بـ 3.13 فتعمل مباشرة، لكن بعض تحديثات كالي الدورية
(rolling) الأحدث ترفع النظام إلى 3.14 — عندها يفشل تثبيت TensorFlow برسالة:
...
### 6ب) صفحة المتصفح تعرض "Problem loading page" أو لوحة القيادة عالقة على "Loading…"
**السبب:** الواجهة تحمّل 17 نموذجًا مدرَّبًا في الذاكرة **قبل** بدء الخادم
(يستغرق 10–40 ثانية). إذا فتحت المتصفح قبل ظهور سطر «Running on
http://127.0.0.1:7860» في الطرفية، فلن يجد الخادم جاهزًا → صفحة الخطأ،
وتبقى لوحة القيادة عالقة على "Loading…".
**الحل:**
1. انتظر في الطرفية حتى ترى: `* Running on http://127.0.0.1:7860`
2. أغلق تبويب «Problem loading page» واضغط **Ctrl+Shift+R** (تحديث قسري)
   في تبويب SOC Console.
3. تأكد أن الخادم مستجيب من طرفية ثانية:
   `curl http://localhost:7860/api/stats`
4. لو ظهر في الطرفية `Address already in use` → توجد نسخة قديمة تعمل:
   `sudo fuser -k 7860/tcp` ثم أعد التشغيل.
> ملاحظة: الإصدار الأخير يفتح المتصفح تلقائيًا **بعد** جاهزية الخادم،
> ويعرض رسالة خطأ واضحة بدل "Loading…" إن تعذّر الاتصال.
